...
import { fetchMoviesByType } from '@/store/actions';
...
const BrowseByTypeContainer = ({ movies, pages, params }) => {
	...
	return (
		<WithMovieListAndPagination data={movies}
																pages={pages}
																currentPage={page}
																pathTo={routeBuilder(routesMap.browseByType, {filter}) + '/'}
		/>
	);
};

const paramsToUseEffect = () => {
	const { filter = 'upcoming', page = 1 } = useParams();
	return [filter, page];
};
...
const mapDispatchToProps = (dispatch, { MovieDatabaseService }) => {
	return bindActionCreators({
		fetchData: fetchMoviesByType(MovieDatabaseService),
	}, dispatch);
};
